...
void GFX_DrawChar_const_size(int x, int y, char chr, ColorType color, uint8_t const_size)
{
	if(chr > 0x7E) return; // chr > '~'
        ...
        for(int8_t j=0; j<font[0]; j++, line >>= 1) // For each pixel in column
        {
            if(line & 1) // Check last pixel in line
            {
            	if(const_size == 1)
            		...
            		GFX_DrawFillRectangle(x+i*const_size, y+j*const_size, const_size, const_size, color); // Or bigger pixel
            }
        }
    }
}
...
void GFX_DrawString_const_size(int x, int y, char* str, ColorType color, uint8_t const_size)
{
	int x_tmp = x;
	...
	while(*str++)
	{
		GFX_DrawChar_const_size(x_tmp, y, znak, color, const_size); // Draw current char

		x_tmp += ((uint8_t)font[1] * const_size) + 1; // Move X drawing pointer do char width + 1 (space)
